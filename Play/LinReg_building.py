...
#Erstellt einen linear Regressor für die Vorhersage von output_var basierend auf den Eingabedaten aus dtree und output_ds. Für einen bestimmten geografischen Punkt.
...
def local_vars_to_one_dim(scen = "historical", variables = ["tas", "pr"], output_var = "mrsol",
    ...
    return local_datatree_to_one_dim(dtree=input_dt,output_ds= output_ds,output_var= output_var,lat_idx= lat_idx,lon_idx= lon_idx,depth= depth,r= r,Month_idx= Month_idx,hist= hist)
...
def global_datatree_to_one_dim(dtree, output_ds, output_var = "mrsol",
    ...
    for dlat_idx in range (min_lat_idx,max_lat_idx):
        ...
        for dlon_idx in range(min_lon_idx,max_lon_idx):
            try:
                new_row.append(local_datatree_to_one_dim(dtree=dtree,output_ds= output_ds,output_var = output_var, lat_idx=dlat_idx, lon_idx=dlon_idx, r = r, depth = depth, hist = hist, prune = prune, start =start, end = end, time_step = time_step, Month_idx = Month_idx))
            except Exception :
                new_row.append(None)
...
def global_vars_to_one_dim(scen = "historical", variables = ["tas", "pr"], output_var = "mrsol",
                            min_lat_idx = 0, max_lat_idx = 40, min_lon_idx = 0, max_lon_idx = 40, depth = 0,r = 0,
                            start ="1850-01-01", end = "1890-01-01", time_step = "1ME", Month_idx = None,  hist = 1,
                            time_bar = False, run_idx = 1,
                            ):
    start_hist = shape.start_with_hist(start=start,time_step= time_step,hist= hist)
    input_dt = shape.load_create_datatree(scenarios=[scen],variables= variables,start= start_hist,end= end,time_step= time_step, run_idx = run_idx)
    output_ds = shape.show_data_set(var=output_var,scen=scen, run_idx = run_idx)
    output_ds = shape.prune_group_ds_timespan(ds=output_ds,start=start,end=end,time_step=time_step,Month_idx= Month_idx)
    return global_datatree_to_one_dim(dtree=input_dt,output_ds= output_ds,output_var= output_var, min_lat_idx=  min_lat_idx,max_lat_idx= max_lat_idx,min_lon_idx= min_lon_idx,max_lon_idx= max_lon_idx,time_bar= time_bar, r = r,  depth = depth, hist = hist,  Month_idx = Month_idx, time_step=time_step)
