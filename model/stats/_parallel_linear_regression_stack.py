...
class ParLinearRegressionStack:
    """Ordinary least squares Linear Regression for xr.DataArray objects."""

    def __init__(self, predictor_dims: list[str] | None = None):
    ...
    def fit(
        ...
        params = _fit_linear_regression_xr(
            ...
            regr_dim = regr_dim,
            predictor_dims=self.predictor_dims,
            weights=weights,
...
def _fit_linear_regression_xr(
    ...
    location_dim: str,
    regr_dim: str,
    predictor_dims: list[str] | None = None,
    weights: xr.DataArray | None = None,
    fit_intercept: bool = True,
    parallel: bool = False
) -> xr.Dataset:
    ...
    if location_dim == "predictor_var" or regr_dim == "predictor_var":
    ...
    # ensure `dim` is equal
    xr.align(predictors_stack, target, join="exact")
    ...
    if parallel: 
        out = np.stack(Parallel(n_jobs=-1)(delayed(_fit_linear_regression_np)( predictors_stack.isel({location_dim: location}),
                                                                            target.isel({location_dim: location}),
                                                                            weights,
                                                                            fit_intercept,)
            for location in range(predictors_stack.sizes[location_dim])),axis=0)

    else:
        out = np.stack([_fit_linear_regression_np(
        predictors_stack.isel({location_dim: location}),
        target.isel({location_dim: location}),
        weights,
        fit_intercept,
        )
        for location in range(predictors_stack.sizes[location_dim])
        ], axis=0)
    
    # remove (non-dimension) coords from target (#332, #333)
    target = target.drop_vars(target[regr_dim].coords)

    # give `out` the same shape as the predictor Datasets
